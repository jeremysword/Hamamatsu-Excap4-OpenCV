...
// OpenCV
#include "opencv2/core.hpp"//opencv.hpp has apparent name collision
...
void unpack_mono12_image(void* pSrcTop, int32 srcRowbytes, void* pDstTop, int32 dstRowbytes, int32 width, int32 height)
{
	...
	/*char* pSrc = (char*)pSrcTop;
	ushort* pDst = (ushort*)pDstTop;

	int i;
	for (i = 0; i < framebytes; i++)
	{
		pDst[2 * i] = pSrc[3 * i] + (pSrc[(3 * i) + 1] & 0xF0);
		pDst[(2 * i) + 1] = pSrc[(3 * i) + 2] + ((pSrc[(3 * i) + 1] & 0x0F) << 4);
	}*/
			...
			else
			{
...
#endif // _EXCAP_SUPPORTS_FRAMEBUNDLE_ !
				{
					...
					if( !failed(err) )
					{
						...
						int32 rowbytes = frame.width * 2;
						int32 framebytes = rowbytes * frame.height;
						char* pImage = new char[framebytes];
						memset(pImage, 0, framebytes);

						unpack_mono12_image(frame.buf, frame.rowbytes, pImage, rowbytes, frame.width, frame.height);
						
						cv::Mat cvimg(frame.height, frame.width, CV_16U, pImage);//12 bit output prevents stuttering
						//ushort* pointer_to_data_start = cvimg.ptr<ushort>();
						//memcpy(pointer_to_data_start, frame.buf, frame.width * frame.rowbytes);
						//imshow("image", cvimg);
						//imwrite("image.tif", cvimg);
						
						
					


						//cv::Mat cvimg(frame.height, frame.width, CV_16U, frame.buf);//12 bit output prevents stuttering
						//ushort* pointer_to_data_start = cvimg.ptr<ushort>();
						//memcpy(pointer_to_data_start, frame.buf, frame.width * frame.rowbytes);
						//imshow("image", cvimg);
					...
					cv::Mat image32f;
					cvimg.convertTo(image32f, CV_32F);

					cv::Mat mu;
					blur(image32f, mu, cv::Size(5, 5));//blur truncates outside 2 pixel edge. blur is not equivalent to imagej mean 3 pixel radius nor 2 pixel radius.  Radius is not a square.
					//imwrite("mu.tif", mu);

					cv::Mat mu2;
					blur(image32f.mul(image32f), mu2, cv::Size(5, 5));

					cv::Mat sigma;
					sqrt(mu2 - mu.mul(mu), sigma);

					cv::Mat coeffvar = sigma.mul(1.0f / mu);
					
					imshow("coeffvar", coeffvar);
					
					
					static int d = 0;

					std::ostringstream name;
					name << "coeffvar_" << d << ".tif";
					std::ostringstream name2;
					name2 << "image32f_" << d << ".tif";

					//imshow("cvimage", cvimage);
					//imshow("mu", mat2gray(image32f));
					//imshow("coeffvar", coeffvar);
					
					//imshow("image32f", coeffvar);
					//imwrite(name.str(), coeffvar);
					//imwrite(name2.str(), image32f);
					d += 1;
